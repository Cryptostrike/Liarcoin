...
#if defined(NDEBUG)
# error "Liarcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x2169d5229186604680ec84c32441e83717f67f81ad2eba0c28ac567ff4210f97");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Liarcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Liarcoin Signed Message:\n";
...
{
    // Liarcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Liarcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Liarcoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Liarcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Liarcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Liarcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Liarcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x2169d5229186604680ec84c32441e83717f67f81ad2eba0c28ac567ff4210f97");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "BBC News 23/08/2014 US anger at China 'air interception' ";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1408754665;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2769792;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Liarcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LiarcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LiarcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LiarcoinMiner(CWallet *pwallet)
{
    printf("LiarcoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running LiarcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("LiarcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LiarcoinMiner, pwallet));
}
